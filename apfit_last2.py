...
## this version approximate pnap : proba of no apfter pulse from dt to n with pnap = np.exp(-L1 -L2) only for the fit, 
## but ignores it to compute the "infinite" sum (with a direct formula)
...
def pn(n, nd, a1, tau1, a2, tau2, mueta):
	...
	return pnc**(R*(n-nd-1)) * (pap + (1-pnc**R))
...
def compute_pap(nd, mueta, a1, tau1, a2, tau2):
	#start at nd rather than nd + 1 ?
	nd = (nd-1)//R 
	pc = 1-np.exp(-mueta) + Pdc
	pnc = 1- pc
	return pnc**(R * (-nd-1)) * ( a1 * summ(1/tau1 - R*np.log(pnc), nd) + a2 * summ(1/tau2 - R * np.log(pnc), nd))


def compute_qber(dt, mu, eta, a1, tau1, a2, tau2, pdc, e0,foF):
	pap = compute_pap(dt, mu*eta, a1, tau1, a2, tau2)
	print(f"pap = {pap}")
	pq = 1 - np.exp(-mu*eta)
		...
		for d in [4, 7]:
			...
			for foF in [fscan[1]]:
				...
				for mu in [0.01,0.1,1.0]:
					...
					res_qber = compute_qber(80*d, mu, eta, a1, tau1, a2, tau2, Pdc, 0.02, foF)
