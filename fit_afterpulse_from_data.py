...
def fit_pap_from_timestamps(path, dead_time_us=6.0, T_max_us=100.0,
    ...
    try:
        ...
        d0   = float(y[100:110].mean()) if len(y) >= 110 else float(y[-1])
...
if __name__ == '__main__':
	...
	T_max_us = 100 #afterpulse_cfg.get('T_max_us', 100.0)
	qber_cal = afterpulse_cfg.get('qber_calibration')
	path = afterpulse_cfg.get("timestamp_file")


	if path:
		res = fit_pap_from_timestamps(path, dead_time_us=6.0, T_max_us=T_max_us, 
			clock_hz=80e6, bin_us=1.0)
		A = res['A']
		tau_us = res['tau']
	else:
	...
	mu=0.01
	print(f"verif (we suppose mu = {mu}):")
	eta=0.01
	Pdc=1.33e-6
	F=0.7
