...
## this version ignores pnap : proba of no apfter pulse from dt to n
## operates at best with dt4_mu0.01
...
def compute_qber(dt, mu, eta, a1, tau1, a2, tau2, a3, tau3, pdc, e0, foF):
	pap = Pap_DT(dt, mu, eta, a1, tau1, a2, tau2, a3, tau3)
	print(f"pap = {pap}")
	pq = 1 - np.exp(-mu*eta)
	#f = 0.26
	#F = 0.78
	#pap_pulse = (pap / (foF * (1 - pap))) *  (pq + foF * pdc)
...
for filename in ["dt4_mu0.01.txt","wide_gates.txt"]:	
	...
	for R in Rlist:
		...
		fscan = np.array([0.26,0.51,0.75])
		Acolor={str(round(fscan[0],3)): 'yellow', str(round(fscan[1],3)): 'violet', str(round(fscan[2],3)): 'red'}
		print(Acolor)
		...
		for d in [4,10]:
			...
			for f in [fscan[0]]:
				...
				for mu in [0.01,0.1,1.0]:
					print("mu=", mu)
					res_qber = compute_qber(d,mu,eta,a111, tau111, a222, tau222, a333, tau333, Pdc, 0.02, f/F)
